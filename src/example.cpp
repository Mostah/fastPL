...
Nombre::Nombre() { premier_ = new Chiffre(0); }

Nombre::Nombre(unsigned long n) { premier_ = new Chiffre(n); }

// Constructeur de copie
Nombre::Nombre(const Nombre &nb) { premier_ = new Chiffre(*nb.premier_); }

// Destructeur de Nombre
Nombre::~Nombre() { delete premier_; }
...
std::istream &operator>>(std::istream &in, Nombre &n) {
  ...
  while (in.good()) {
    ...
    if (std::isdigit(c)) {
      ...
      Nombre::Chiffre *cfLu =
          new Nombre::Chiffre(d); // le chiffre qui vient d'être lu
